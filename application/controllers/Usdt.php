...
class Usdt extends MY_Controller {

	public function __construct()
        {
                parent::__construct();
                $this->load->helper('url_helper');
                $this->load->model('Usdt_coin_model');
                $this->load->model('Btc_coin_model');
                $this->rpc_url = "http://localhost:".$this->config->item('usdtPort');
	...
	private function unlockWallet() {
            $params = [$this->config->item('usdtPWD'),30];
        ...
        public function newAccount() {
            ...
            for($i=0; $i<$this->config->item('loopTime'); $i++) {
                ...
                $this->Btc_coin_model->create_account($data);
            }
           $this->lockWallet();
           echo "成功创建了 ".$i." 个BTC新账号:".date('Y-m-d H:i:s')."\r\n";
        }
        
        public function sendBTC($from_account,$from_address,$to_address,$amount) {
            $accounts = $this->call("getaddressesbyaccount",[$from_address]);
            $unspents = $this->call("listunspent",[1,999999,$accounts]);
            if (empty($unspents)) {
                exit("热钱包交易费用不足!\r\n");
            }
            $hw_balance = 0; // from钱包btc余额
            $remain_balance = 0; // from钱包剩余的余额
            $send_array = []; // 发送名单
            $input_length = count($unspents); // input长度
            $output_length = 2; // output长度，+1代表剩余的余额作为一个output返还到from钱包
            foreach ($unspents as $unspent) {
            ...
            if ($remain_balance < 0) {
             ...
             echo "from钱包余额:".$hw_balance."\r\n";
             //校验地址
            $validate = $this->call("validateaddress",[$to_address]);
            if ($validate['isvalid'] == false) {
                exit("转出地址有误!\r\n");
            }
            $remain_balance = calculate($remain_balance, $amount, 'bcsub');
            if ($remain_balance < 0) {
                exit("热钱包余额不足!\r\n");
            }
            $send_array[$to_address] = $amount;
            $send_array[$from_address] = $remain_balance;
            $this->unlockWallet();
            $tx_hash = $this->call("sendmany",[$from_account,$send_array]);
            $this->lockWallet();
        ...
        public function withdraw() {
            ...
            foreach($res as $item) {
                ...
                if ($validate['isvalid'] == false) {
                    ...
                    echo "转出地址有误!\r\n";
                    $this->Usdt_coin_model->update_withdraw_log($data,$id);
                    continue;
                } 
