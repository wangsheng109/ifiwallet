	...
	public function __construct()
        {
                ...
                $this->load->model('Dash_coin_model');
                $this->rpc_url = "http://localhost:".$this->config->item('dashPort');
                ...
                if($this->coin_id!=$this->config->item('dash_coin_id')){
                    exit("coin id 数据库与配置文件不一致! ".date('Y-m-d H:i:s')." \r\n");
                }
        }
        
	private function unlockWallet() {
            $params = [$this->config->item('dashPWD'),30];
        ...
        public function newAccount() {
            ...
            for($i=0; $i<$this->config->item('loopTime'); $i++) {
                ...
                $this->Dash_coin_model->create_account($data);
            }
           $this->lockWallet();
           echo "成功创建了 ".$i." 个DASH新账号:".date('Y-m-d H:i:s')."\r\n";
        }
        ...
        public function deposit() {
            ...
            $res = $this->Dash_coin_model->get_cold_wallet();
            foreach($res as $item) {
             ...
             // wrap content
                ...
                foreach($txs as $tx) {
                    ...
                    $this->Dash_coin_model->insert_db("coin_deposit_log",$log_db_data);
                    $db_amount = calculate($db_amount,$tx['amount'],"bcadd");
                    $cold_wallet_db['balance'] = $db_amount;
                    $this->Dash_coin_model->update_cold_wallet($cold_wallet_db,$item['id']);
                    $complete +=1;
                }
            }
        ...
        public function depositConfirm() {
            $deposits = $this->Dash_coin_model->get_deposit_log();
            foreach($deposits as $item) {
                ...
                if (isset($tx['confirmations']) && $tx['confirmations'] > 0) {
                    ...
                    $this->Dash_coin_model->update_deposit_log($data,$item['id']);
        ...
        public function withdraw() {
            $complete = 0;
            $res = $this->Dash_coin_model->get_withdraw_log(200);
            if (empty($res)) exit("暂无提现\r\n");
            // 获取热钱包的unspent和计算热钱包余额
            $accounts = $this->call("getaddressesbyaccount",[$this->config->item('dashPayAccount')]);
            ...
            foreach($res as $item) {
                ...
                if ($validate['isvalid'] == false) {
                    ...
                    echo "转出地址有误!\r\n";
                    $this->Dash_coin_model->update_withdraw_log($data,$id);
                    continue;
                } 
            ...
            try{
                if (! empty($send_array)) {
                    $send_array[$this->config->item('dashPayAddress')] = $remain_balance;
                    ...
                    foreach ($withdrawed_array as $item) {
                        ...
                        $data['status'] = 210;
                        $data['update_time'] = time();
                        $this->Dash_coin_model->update_withdraw_log($data,$item['id']);
                        $complete += 1;
                    }
                } else {
                    echo "send_array 为空! \r\n";
...
//                        $this->Dash_coin_model->update_withdraw_log($data,$item['id']);
        ...
        public function withdrawConfirm() {
            $res = $this->Dash_coin_model->get_withdraw_log(210);
            $complete = 0;
            foreach($res as $item) {
                $id = $item['id'];
        ...
        public function withdrawSys() {
            ...
            foreach ($unspents as $unspent) {
                if (!isset($unspent['account']) || empty($unspent['account']) || $unspent['account'] == $this->config->item('dashPayAccount') || in_array(strtolower(strval($unspent['address'])), [strtolower(strval($this->config->item('dashPayAddress'))), strtolower(strval($this->config->item('dashWithdrawTo')))])){
                    echo "account : ".$unspent['account']."\r\n";
                    echo "address : ".$unspent['address']."\r\n";
                    echo "系统提现账号有误\r\n";
                    continue;
                }
                $count = $this->Dash_coin_model->check_deposit_log_duplicate($unspent['txid'],$unspent['address']);
            ...
            try {
                if(!empty($withdrawed_array)){
                    $threshold = $this->Dash_coin_model->get_threshold();
                    $fee = calculateFee($input_length, $output_length);
                    ...
                    $send_array[$this->config->item("dashWithdrawTo")] = $remain_balance;
                    $tx_base = $this->call("createrawtransaction",[$filter_unspents, $send_array]);
                    $this->unlockWallet();
                    $sign_hex = $this->call("signrawtransaction",[$tx_base]);
                    if(!$this->call("settxfee",[$this->config->item('FEE_PER_KILOBYTE')])){
                    ...
                    if (!empty($tx_hash)) {
                        ...
                        foreach ($withdrawed_array as $address => $amount) {
                            $cold_wallet = $this->Dash_coin_model->get_cold_wallet(['address'=>$address],true);
                            if(empty($cold_wallet)||!isset($cold_wallet[0])) {
                                var_dump($cold_wallet);
                                echo "查询地址冷钱包余额为空: ".$address." :".date('Y-m-d H:i:s')."\r\n";
                                continue;
                            }
                            $data['cid'] = $cold_wallet[0]['id'];
                            $data['coin_id'] = $this->config->item('dash_coin_id');
                            $data['address'] = $address;
                            $data['tx_hash'] = $tx_hash;
                            $data['status'] = 210;
                            $data['create_time'] = time();
                            $data['to_address'] = $this->config->item('dashWithdrawTo');
                            $data['amount'] = $amount;
                            $this->Dash_coin_model->insert_withdraw_sys_log($data);
                        }
                        $complete += 1;
                    }
                }
        ...
        public function withdrawSysConfirm() {
            try{
                $res = $this->Dash_coin_model->get_withdraw_sys_confirm();
                $send_fee_array = array(); 
                $update_array = [];
                $complete=0;
                foreach($res as $item) {
                    $cold_wallet = $this->Dash_coin_model->get_cold_wallet(['address'=>$item['address']],true);
                    if(empty($cold_wallet)||!isset($cold_wallet[0])) {
                        continue;
                    }
                    $id = $item['id'];
                    ...
                    $this->Dash_coin_model->update_cold_wallet($cold,$item['cid']);
                    $sys_log_data['status'] = 280;
                    $this->Dash_coin_model->update_sys_withdraw($sys_log_data,$id);
                    $complete++;
                }
