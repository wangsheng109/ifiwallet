...
class Doge extends MY_Controller {

	public function __construct()
        {
                parent::__construct();
                $this->load->helper('url_helper');
                $this->load->model('Doge_coin_model');
                $this->rpc_url = "http://localhost:".$this->config->item('dogePort');
                ...
                if($this->coin_id!=$this->config->item('doge_coin_id')){
        ...
        public function newAccount() {
            ...
            for($i=0; $i<$this->config->item('loopTime'); $i++) {
                ...
                $this->Doge_coin_model->create_account($data);
            }
           echo "成功创建了 ".$i." 个DOGE新账号:".date('Y-m-d H:i:s')."\r\n";
        }
        ...
        public function deposit() {
            ...
            $res = $this->Doge_coin_model->get_cold_wallet();
            foreach($res as $item) {
             ...
             // wrap content
                ...
                foreach($txs as $tx) {
                    ...
                    $this->Doge_coin_model->insert_db("coin_deposit_log",$log_db_data);
                    $db_amount = calculate($db_amount,$tx['amount'],"bcadd");
                    $cold_wallet_db['balance'] = $db_amount;
                    $this->Doge_coin_model->update_cold_wallet($cold_wallet_db,$item['id']);
                    $complete +=1;
                }
            }
        ...
        public function depositConfirm() {
            $deposits = $this->Doge_coin_model->get_deposit_log();
            foreach($deposits as $item) {
                ...
                if (isset($tx['confirmations']) && $tx['confirmations'] > 0) {
                    ...
                    $this->Doge_coin_model->update_deposit_log($data,$item['id']);
        ...
        public function withdraw() {
            $complete = 0;
            $res = $this->Doge_coin_model->get_withdraw_log(200);
            if (empty($res)) exit("暂无提现\r\n");
            // 获取热钱包的unspent和计算热钱包余额
            $accounts = $this->call("getaddressesbyaccount",[$this->config->item('dogePayAccount')]);
            ...
            foreach($res as $item) {
                ...
                if ($validate['isvalid'] == false) {
                    ...
                    echo "转出地址有误!\r\n";
                    $this->Doge_coin_model->update_withdraw_log($data,$id);
                    continue;
                } 
                ...
                if (isset($send_array[$item['coin_address']])) {
                    ...
                    $send_array[$item['coin_address']] = floatval($item['coin_actual_amount']);
                }
                $withdrawed_array[] = $item;
            }
            try{
                if (! empty($send_array)) {
                    $send_array[$this->config->item('dogePayAddress')] = $remain_balance;
                    $tx_base = $this->call("createrawtransaction",[$filter_unspents, $send_array]);
                    $sign_hex = $this->call("signrawtransaction",[$tx_base]);
                    if(!$this->call("settxfee",[floatval($this->config->item('FEE_PER_KILOBYTE'))])){
                            exit("交易费设置异常!\r\n");
                    }
                    ...
                    foreach ($withdrawed_array as $item) {
                        ...
                        $data['status'] = 210;
                        $data['update_time'] = time();
                        $this->Doge_coin_model->update_withdraw_log($data,$item['id']);
                        $complete += 1;
                    }
                } else {
                    echo "send_array 为空! \r\n";
...
//                        $this->Doge_coin_model->update_withdraw_log($data,$item['id']);
        ...
        public function withdrawConfirm() {
            $res = $this->Doge_coin_model->get_withdraw_log(210);
            $complete = 0;
            foreach($res as $item) {
                $id = $item['id'];
        ...
        public function withdrawSys() {
            ...
            $unspents = $this->call("listunspent",[1, 999999]);
            if (empty($unspents)) {
                exit("暂无记录\r\n");
            }
            var_dump($unspents);
            foreach ($unspents as $unspent) {
                if (! isset($unspent['account']) || empty($unspent['account']) || $unspent['account'] == $this->config->item('dogePayAccount')  || in_array(strtolower(strval($unspent['address'])), [strtolower(strval($this->config->item('dogePayAddress'))), strtolower(strval($this->config->item('dogeWithdrawTo')))])){
                    echo "系统提现账号有误\r\n";
                    continue;
                }
                $count = $this->Doge_coin_model->check_deposit_log_duplicate($unspent['txid'],$unspent['address']);
            ...
            try {
                if(!empty($withdrawed_array)){
                    $threshold = $this->Doge_coin_model->get_threshold();
                    $fee = calculateFee($input_length, $output_length);
                    ...
                    $send_array[$this->config->item("dogeWithdrawTo")] = floatval($remain_balance);
                    $tx_base = $this->call("createrawtransaction",[$filter_unspents, $send_array]);
                    $sign_hex = $this->call("signrawtransaction",[$tx_base]);
                    if(!$this->call("settxfee",[floatval($this->config->item('FEE_PER_KILOBYTE'))])){
                            exit("交易费设置异常!\r\n");
                    ...
                    if (!empty($tx_hash)) {
                        ...
                        foreach ($withdrawed_array as $address => $amount) {
                            $cold_wallet = $this->Doge_coin_model->get_cold_wallet(['address'=>$address],true);
                            if(empty($cold_wallet)||!isset($cold_wallet[0])) {
                                continue;
                            }
                            $data['cid'] = $cold_wallet[0]['id'];
                            $data['coin_id'] = $this->config->item('doge_coin_id');
                            $data['address'] = $address;
                            $data['tx_hash'] = $tx_hash;
                            $data['status'] = 210;
                            $data['create_time'] = time();
                            $data['to_address'] = $this->config->item('dogeWithdrawTo');
                            $data['amount'] = $amount;
                            $this->Doge_coin_model->insert_withdraw_sys_log($data);
                            $complete += 1;
                        }
                    }
                }
        ...
        public function withdrawSysConfirm() {
            try{
                $res = $this->Doge_coin_model->get_withdraw_sys_confirm();
                $send_fee_array = array(); 
                $update_array = [];
                $complete=0;
                foreach($res as $item) {
                    $cold_wallet = $this->Doge_coin_model->get_cold_wallet(['address'=>$item['address']],true);
                    if(empty($cold_wallet)||!isset($cold_wallet[0])) {
                        continue;
                    }
                    $id = $item['id'];
                    ...
                    $this->Doge_coin_model->update_cold_wallet($cold,$item['cid']);
                    $sys_log_data['status'] = 280;
                    $this->Doge_coin_model->update_sys_withdraw($sys_log_data,$id);
                    $complete++;
                }
